...
author: "Jane Ho / Yuhan Jiang (github)"
...
project:
  type: website
  pre-render: "echo let the render begin!"
  post-render:
    - "cp -rv InterlabsCodeGuidance.html index.html"
    - "cp -rv InterlabsCodeGuidance_files/. index_files/"
---
...
|R, RStudio|data analysis| |latest versions comes packaged with Quarto|
|Quarto|publishing system|1.6.6|this version includes page orientation toggle; add homebrewed pt zip|
|Python|file manipulation| | |
|[Visual Studio2022 - Community](https://visualstudio.microsoft.com/)| | | |
|[SSMS](https://learn.microsoft.com/en-us/sql/ssms/download-sql-server-management-studio-ssms?view=sql-server-ver16)| | | |
|[SQL Server - Developer](https://www.microsoft.com/en-ca/sql-server/sql-server-downloads)| | | |
...
## Quarto 
Quarto is an open-source scientific and technical publishing system which should available "in-the-box" with current versions of RStudio.  (If running an older version, please download the standalone quarto executable and provide instruction via terminal to use quarto)
Quarto syntax is based on markdown ~ refer to [primer](https://quarto.org/docs/authoring/markdown-basics.html).  It is currently used for published reports in the program, e.g. this documentation paper, and the proficiency test reports (basecode ~ testReport-typst.qmd).  

Specific to the proficiency test reports, elements are required for compilation are (besides key workflow inputs depicted at @fig-workflow) as shown:  
- the custom extension "pt" must be added.  Extract zip to base code, in the RStudio terminal run: quarto use template pt, then select no to creating a new subdirectory, select yes to use the template, and yes for replacing basecode.qmd  
- qmdElem folder must have rds objects generated from base code and misc. images  
- assets folder holding the labstats logo
  
To publish proficiency test reports, select render as pt-typst for pdf, and render as html for standalone html.  
...
## .NET MVC (C#)
1. **What is MVC Paradigm?**
- Model
    - Represent the application's data as objects.
    - Interact with the database to add, edit, view or delete data.
    - Contain classes that define the data structure.

- View
    - Represent the presentation layer (UI).
    - Display the data from Model layer to user.
    - Consist of HTML, CSS, and Razor syntax to create pages.

- Controller
    - Connect Model and View.
    - Handle user input, process and return the appropriate View.
    - Contain business logic.

2. **How MVC works in .NET**
  - **Request Flow**: When a user interacts with an ASP.NET MVC application, the request is routed to a specific controller and action method based on the URL.
  - **Controller Action**: Process the request and interacts with the Model to retrieve or update data.
  - **Returning a View**: The controller action then returns a View, which is rendered as HTML and sent back to the user's browser. 


3. **Create Branch in Visual Studio**
- Clone the [ PTplatform Repository ](https://github.com/InterlabsOCWA/PTplatform).
- Navigate the file, and open the project by clicking the  "PTplatform.sln".
- From the **Git** menu, select **New Branch**. 
![VS Git Menu](img/git-menu-new-branch.png)
- In the **Create a new branch** dialog box, enter a branch name.
![VS Git New Branch](img/git-create-new-branch-dialog.png)
  - Find **master** from **Based on:** dropdown and click it.
- Click **Create**. You are now on the new branch.

4. **Commit & Push in Visual Studio?**
- At the top Menu, Click **Git**.
- Click **Commit or Stash...**.
  - You can see all the changes you've made under Changes.
- Write a commit notes in the **Enter a message**  ex. Home UI implemented.
![VS Git Commit](img/git-changes-window.png)
- Click **Commit All** button.
- For push, click this arrow pointing up![Push btn](img/push.png).


## Postgres SQL database set-up (subject to change)
N.B. database is local for now - local version being shared via "back-up and restore" method as described in https://stackoverflow.com/questions/30043392/copying-a-postgres-database-from-one-server-to-another  
...
## Postgres database operation {#db_op}
1. To start Postgres server, use the below command adapting the path to where you've previously set as database storage above:
```sh
pg_ctl -D C:/Users/hoja/AppData/Local/PostgresSQL/pgsql/data -l logfile start
```
The command line should then respond with "waiting for server to start.....done.  Server started".  If using pgAdmin to access, follow that GUI's dropdown hierarchy.  If using shell, use following steps:  

2. Access client using
```sh
psql -U postgres
```sh
...
iii.  if prompted, the password is the same as the user name (or whatever the db is set too, e.g. interlab)
...
::: {.callout-tip}
## Some useful PSQL one-liner's

export the table to csv to sort/filter/copy into the other spreadsheets)
```sh
\copy dt_method_summary TO 'dt_method_summary.csv' CSV HEADER
```
use this to check the submit to qaqc summary worked for the last lab - if uploaded/submitted by ascending order, the last labs would show first in descending order)
```sh
SELECT * from dt_qaqc_summary ORDER BY "Lab_ID" DESC LIMIT 10;
```
:::
...
::: {.callout-note} 
You can bind this linting process to a keyboard shortcut in RStudio in the addin's window, with Shift+Alt+L recommended *
...
- every modular piece of code that performs an action, transforming an input to output should be contained in a function
- functions (or a related family thereof)
...
The happy path is the path through a system where everything is as expected.
...
Please switch to web branch before usage.

### main branch
This branch is reserved for website deployment (on shinyapps).  It is in hiatus since sample Rey and will not work with later rounds.  It was tested successfully as a route for report submittal (using Outlook SMTP) but does not include database connection. 

### web branch  *In use*
This is the current branch, with consideration of template changes in Tarkin.  It has not been tested for report submittal functionality (those code chunks may still be mapped to older template locations).  
Prior to usage, decide if you are at the point of writing to database (i.e. have cleaned results).  If so, set first chunk eval_db to TRUE and ensure the [database](#db_op) is running prior to starting.  If not (i.e. just checking/cleaning raw results), set eval_db to FALSE and there is no need to run database concurrently.   

### db branch  
This branch has been incorporated with web branch as of Sample Tarkin and is no longer in use.  

## Report split-generation [obsolesced since end of MECP WSI program]
1. Use new sample notes which has an additional field for report group, which must be filled as pt, interlab, or both.  
![sample notes spreadsheet](img/sampleNotesRptGrp.png)  
2. In Tarkin round of github (currently a development branch), the base code is altered 
2a. in config chunk at start, define reportType variable as pt or interlab (~line 70)  
2b. chunk 2d filters out by report group to generate the df "sample_status" which generates df "anonymous_ID_inhibition", which is then joined to dt_interlab.  This should be changed to **inner join** to cut out labs not in the group.  

::: {.callout-note}
TODO change to **inner join**  
N.B.*maybe cut out fnames for quicker read?*
::: 
...
A September 2023 successful Shiny-based regulatory [submission](https://pharmaverse.github.io/blog/posts/2023-08-14_rhino_submission_2/rhino_submission_2.html) to the FDA can be considered a litmus test of recognition for the use of R & Shiny-built apps in compliance work requiring statistical review and evaluation. 

The current QAQC app is built using flexdashboard with Shiny reactive elements.  The advantage is there is a "single source of truth" i.e. no separated ui & server, front & backend files.  This allows time to be focused on building functionality rather than app-building and synchronizing variables/functions across front and back-end file sets.  However, in consideration of expansion with other website functions (e.g. user authentication, payment, multi-page navigation, routing), it is unlikely the flexdashboard can support future needs.  

Furthermore, future staff slated to support the expansion is expected to have a full-stack dev background - hence the code needs to move towards more formal organization in alignment with typical full stack practices.  A pre-packed framework rather than our current dashboard/configuration using individually curated packages may be more conducive for maintenance.

### Available Framework options
The most prominent R Shiny packages for providing a opiniated framework for full stack Shiny apps are Rhino, Golem, and Leprechaun (as reviewed by [G.Kenneth for Appsilon](https://appsilon.com/rhino-vs-golem-vs-leprechaun/)).  
  
![available frameworks](https://wordpress.appsilon.com/wp-content/uploads/2023/11/Rhino_v_Golem_V_Lepre.webp)
**Image source: [Appsilon](https://appsilon.com/rhino-vs-golem-vs-leprechaun/)**

Leprechaun is not further evaluated as it does not have a built-in testing framework, which is desirable for our use.  Different from the other two options, it does not add extraneous (including itself) as a dependency, which is advantageous for the size and fragility of applications.  It may be worthwhile exploring at a second pass.

Rhino is the "new kid on the block", having been released by Appsilon early 2022. It is underpinned by the Box package.  Golem, released by ThinkR, is the more established framework, running under the paradigm of package creation, including the associated rigourous testing procedures.

Both Rhino and Golem are accompanied by their respective "universe" of supporting packages which facilitate shiny use for production, such as React and Docker enablement, state management, benchmarking.  The availability of these tools would be very helpful for the development of the interlabs QAQC app and hence, these two frameworks were evaluated further.
...
The folder structure organization are follows:  
  
Golem folder structure
```
golemappy
├───dev
├───inst
...
└───tests
    ...
    └───testthat
```    
Alternatively, Rhino exclusive of its E2E tests and config settings, conceptually broken down:  
![Rhino, exclusive of its E2E tests and config settings, as conceptually broken down by Jinhwan Kim](https://miro.medium.com/v2/resize:fit:1400/format:webp/1*nMDRb0pu3uvCTalCYY_Lkw.png)
Source: ["How to make a cheatsheet for Rhino" (Kim, J.) ](https://jhk0530.medium.com/how-to-make-cheatsheet-for-r-package-rhino-5cdfa22c308e)


**Dependency management** : The Rhino framework uses renv (the modern version of packrat) to control versions of package dependencies used.  At deployment though, it still requires packrat.  Golem (as learnt in the pharmaverse workshop) may also use renv, although that was included in its template set-up and renv must be separately initialized.

**Learning Curve & guidance** : Both frameworks have good documentation.  However (as described in detail in next section's time-boxed trials), golem has very thorough and walkthrough-like comment-prompts in the script templates, nearly adequate to fully code-along just within the script.  Whereas, Rhino provides no in-script guidance, just a basic example (similar to npx create-react-app).  It is necessary to working with the docs open by its side.  Just by the sheer extent of the default template folder structure, it is likely to take more time to figure out and update all the corresponding elements.  Its similarity to other full stack frameworks will be conducive to navigation and more accessible to those with a software engineering background.  

![guidance in script during dev - golem on left, Rhino on right](src/framework_scripts.png)


**How to render (in development) & How to deploy** 
...
Rhino render command  
```
shiny::runApp()
```

Golem render command
```
...
Both are deployed using from the app.R in their main directory, which allows the use of the "publish" shortcut in the RStudio IDE.  Golem's deploy step R file provides testing commands that should be & are easily run pre-deployment with clear console report-out on test results.  

Rhino deploy command
```
# Rhino / shinyApp entrypoint. Do not edit.
rhino::app()
```
  
Golem deploy command (many options provided...just used the test portion then IDE shortcut)
```
# Test your app
...
Helpful & confidence-building test results from Golem's pre-deployment checks:  
![Golem pre-deployment testing](src/framework_test.png)


#### Rhino vs. Golem
In discussion of how to approach exploring these frameworks with a mentor (not fluent in R), one advice was to check the number of downloads which is indicative of uptake and available community support - e.g. in the frameworks the mentor would advise for python (flask), downloads would be in the millions.  Official downloads counts of Rhino and Golem are available from CRAN.  

A roughly^ 1-hour time-boxed trial was conducted in both frameworks to "wet the toes".  The objective was to create an app with two plots, using a native dataset (easy proof-of-concept), then using the CRAN download counts for the two packages (to see how to add a second module).  The build was done on a Windows laptop, R 4.3.2, RStudio as IDE, with Node v.16 (required for Rhino).  As a stretch step beyond the timebox, an existing Shinyapps.io account was used for deployment.  Datasets used are the publicly available CRAN downloads count, R's in-the-box iris test data (and Rhino's similar test rhinos test until I figured out how to import iris into Rhino).

Rendered results are as follows, with Golem on left and Rhino on right (missing its title as Rhino requires a box::use call to be even able to add a h1 heading which was figured out after the 1-hour limit).  Both plots for Rhino were made post the 1-hour timebox due to not understanding the package call syntax (which meant both datasets and ggplot failed to work).  The top plots were rendered using iris dataset, in base R for Golem and ggplot (used it as a step to figure the syntax) for Rhino, respectively.  Bottom plots were the CRAN downloads data plotted using ggplot (much higher downloads for older and well-established Golem!)

![rendered apps - golem on left, Rhino on right](src/framework_rendered.png)

**overall impressions**  
  
| Aspect                                          | Golem                                                                                                                                                                                                                                                                                                                                                           | Rhino                                                                                                                                                                                                                                                                                                                                                                                  |
|-------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Objective reached?                              | 50%.  Plotted a standard dataset (iris) graph only.  The logic for the downloads count was written (and works outside of the app) but plot did not render.                                                                                                                                                                                                      | 50% Plotted (a different) standard dataset (rhino's rhinos) graph since the usual iris was not load-able.  Second graph using ggplot was not rendered (see syntax issue)                                                                                                                                                                                                               |
| Sticking-point                                  | namespace mistake - within the function module file, the name for the second graph was spelt differently between the namespace taglist and server function output                                                                                                                                                                                               | Syntax to use functions from packages.  Had trouble calling the usual in-the-box default iris dataset and using ggplot.                                                                                                                                                                                                                                                                |
| Actual finish time                              | ~1:30 with extra time spent on i) sticking-point ii) adding renv package lock file a posteriori (did not realize it did not generate by default)                                                                                                                                                                                                                | ~1:30 sticking point with the syntax                                                                                                                                                                                                                                                                                                                                                   |
| Structure                                       | For user without a basis in R package development, this was actually more foreign than Rhino.  Needed to refer to a previously viewed video walkthrough and read the in-script guidance comments.                                                                                                                                                               | Very similar to js react & express structure                                                                                                                                                                                                                                                                                                                                           |
| Syntax                                          | Highly similar to daily R coding.  Commented-out examples in default files were extremely helpful.                                                                                                                                                                                                                                                              | Spent majority of time grappling with syntax to be able to use ggplot (tutorials use echarts4r, probably a nudge thereto given its speedier render) and even the basic iris dataset.  Needed to understand the box package and calling packages' functions (even ones not normally thought of as functions, e.g. ggplot's aes) in the form of package$function.  Same for dataset$iris |
| Workflow                                        | Extremely clear using the prescriptive steps provided as in-script comments.  Uses three lines of code in the run_dev file to render each line.                                                                                                                                                                                                                 | Less clear, coder need to keep track but has the option to render app more simply any time.  May be meant to be flexible for task distribution in a team                                                                                                                                                                                                                               |
| Mental effort, open files                       | 5 (ui, server, 1st graph module, 2nd graph module, run_dev). ui & server files are separate (and of course the modules) so there is a bit of flipping back and forth to ensure the correct modules aspects are called between files.  There are useful auto-generated scripts that comes with generated modules directing copy-this-to-ui & copy-that-to-server | 3 (main.R, 1st graph module, 2nd graph module) for this simple use, a single main.R file holds both ui and server scripts so there is one less open file to juggle.                                                                                                                                                                                                                    |
| Size (pre-deployment, completed with two plots) | 140 KB, 60 files, 27 folders.  The renv is substantially lighter containing no actual libraries, only a json settings record of library versions                                                                                                                                                                                                                | 3.82 MB, 134 files, 49 folders.  More than half of which (2.40 MB, 83 files, 18 folders) due to the renv holding actual libraries scripts (only two "used" were explicitly called in code, but all their sub-libraries are included)                                                                                                                                                   |
| Deployment                                      | Workflow required test process (~5 minutes) and with clear reports on asserts pass/fail.  Deployment to shinyapps.io straightforward using IDE shortcut.  Failing functions namespace (seems to be library in local vs. global namespace) so deployment did not render plots.                                                                                                | No "shepherding" to pre-deployment testing but deployment successful.  Deployment to shinyapps.io straightforward using IDE shortcut.                                                                                                                                                                                                                                                  |

The result of the deployment stretch goal is shown below, with Golem on left, Rhino on right.  Golem's included pre-deployment tests had several warnings about packages not being in the global environment. Hence its deployment is certainly mea culpa (lack of understanding of namespacing?) and not due to the framework, which did kindly & rather clearly gave specific warnings.  Rhino, on the other hand, did even render until the syntax issues were resolved - and when it deployed, it was WYSIWYG from the render. 
![deployed apps - golem on left, Rhino on right](src/framework_deployed.png)

**What is exciting**
The testing structure provided in both frameworks is a key attractor.  Unit testing for both appears to be the same, based on Hadley's testthis library; Rhino additionally provides end-to-end testing using Cypress which Golem does not.  Golem's included unit tests pre-deployment is excellent.  Rhino's other selling point is integrated github CI which includes e.g. linting.

**Common concepts to take-away and potential challenges explore further**   
- namespace, modularization in Shiny common to both frameworks - avoiding namespace collisons across different areas of the app and encapsulating distinct app interfaces   
- allowance for performative coding e.g. Do proxy update using just client-side based on key events to avoid server calls  
- Rhino requires Node js > version 12.  Are there conflicts between Rhino and different post-12 versions of node js?  friction points with react (or okay as long as you use Rhino's own react package)  
- Rhino's specificity and use of box seems to create much more verbose script.  Explore if there are options to condense, otherwise existing ggplot figures will be extremely lengthy if migrated over
- providers (Appsilon, ThinkR) & community support  
- ease-of-approach to user authentication and payments (which are major feature needs)  

# PT Platform

> A comprehensive platform for managing Proficiency Testing (PT) rounds, protocols, and quality control validation for laboratories.

## 🔐 Registration & Authentication

::: {.callout-note title="Features"}
- Users can register with: **First Name, Last Name, Lab, Email, and Password**.
...
The protocol system is modular. A user-defined protocol consists of:

### Sample Processing

- Users can **add** or **delete** sample processing steps.
- Supports custom naming and description.

### Extraction

- Users can **add** or **delete** extraction steps.
- Allows entry of extraction details and conditions.

### Quantification

- Supports both **Singleplex** and **Multiplex**.
- Users can:
  - Add, edit, or delete quantification steps.
  - Modify **target genes**.
  - Add multiple targets for **Multiplex** assays.

### Protocol Assembly

::: {.callout-important title="Immutable Protocols"}
Once saved, a protocol cannot be edited. This ensures version control and reproducibility.
:::

- Combines sample processing, extraction, and quantification.
- Protocol types:
  - `Primary`
  - `Secondary`
  - `Candidate`
- Users can add protocols.
- User can only delete protocols if it is not used in the result.

## ✅ Quality Control Validation

### Whole Process Control (Optional)

- No specific validation rules; optional for documentation.

### Whole Process Blank (Mandatory)

- If result ≠ `MAX` (case-insensitive): **Yellow background warning**.
- If non-numeric value: **Validation fails** (Red background).

### Inhibition Control (Mandatory)

- Dynamic form based on inhibition type:
  - `Endogenous` or `Exogenous`
- Auto-calculates:
  - Average values for **Undiluted** and **Diluted**
  - Final **Inhibition status**

### Standard Curves (Mandatory)

- Add both **registered** and **unregistered** target-assay pairs.
- Supports:
  - `Long-Term`
  - `Short-Term`
...
- Dynamic chart display:
  - Shows **slope**, **Cq points**, and **invalid points** (red border).

### No Template Control (Mandatory)

- Same validation as WPB:
  - Result ≠ `MAX`: Yellow warning
  - Non-numeric: Red failure

### Positive Controls (Mandatory)

- All results must be numeric.
- Character values are rejected.

## 📊 Result Submission

- Preloaded Target–Assay pairs from registered protocols.
- Users can:
  - Add, edit, delete Target–Assay rows.

::: {.callout-tip title="PCR-Specific Requirements"}
...
- **dPCR**:
  - 3 **Biological Replicates**
  - Each with 2 **Technical Replicates**

:::

Form behavior changes dynamically based on the selected PCR type.

## 👥 User Roles and Permissions

| Role         | Description |
|--------------|-------------|
| **Super Admin** | - Manage PT rounds<br>- View labs<br>- Download results as JSON format |
| **Lab Admin**   | - Manage lab and staff (to be implemented)<br>- Submit results<br>- Download results as JSON format |
| **Lab Staff**   | - Submit results<br>- Download results as JSON format |
| **TAP**   | - tbd |
...
::: {.callout-note title="Ideas for Future Enhancements"}
- Enable Lab Admins to manage their lab staff directly through the platform.
- Support uploading of analyzed results in PDF or HTML format.
- Streamlined Lab Admin and Staff registration, triggered upon administrative approval.
:::
