...
Since some functions have same purpose (e.x. generate plots), we can organize them all into an external r file (see example below) and use **source("path/filename.R")** to import all functions in that r file in base code file then call them to use.
![](img/functionFile.png)

::: {.callout-note}
Note: If you edit something in the external file, remember to save it and rerun the **source()** line to reload the updat file.
:::
